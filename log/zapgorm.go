...
func (l Logger) Trace(ctx context.Context, begin time.Time, fc func() (string, int64), err error) {
    ...
    case err != nil && l.LogLevel >= gormlogger.Error && (!l.IgnoreRecordNotFoundError || !errors.Is(err, gorm.ErrRecordNotFound)):
        sql, rows := fc()
        logger.Error("", zap.Error(err), zap.Int64("elapsed", elapsed.Milliseconds()), zap.Int64("rows", rows), zap.String("sql", sql))
    case l.SlowThreshold != 0 && elapsed > l.SlowThreshold && l.LogLevel >= gormlogger.Warn:
        sql, rows := fc()
        logger.Warn("", zap.Int64("elapsed", elapsed.Milliseconds()), zap.Int64("rows", rows), zap.String("sql", sql))
    case l.LogLevel >= gormlogger.Info:
        sql, rows := fc()
        logger.Debug("", zap.Int64("elapsed", elapsed.Milliseconds()), zap.Int64("rows", rows), zap.String("sql", sql))
    }
}
